...
if ( !isset($_POST['emailaddr']) || !empty($_POST['emailaddr']) ) {
	$input['email'] = $_POST['emailaddr'];
} else {
	$input['email'] = '';
}

if ( isset($_POST['send_type']) || !empty($_POST['send_type']) ) {
	$input['send_type'] = $_POST['send_type'];
} else {
	$input['send_type'] = 'normal';
}
...
function additional_emails ($send_type) {
	...
	} elseif ( $send_type == 'testnocc' ) {
		return "";
	}
}

function bccEmailHeader ($send_type) {
...
function weekly_format_mail4($client = '', $inputArr) {

	$send_type = $inputArr['send_type'];
	$toEmail = $inputArr['email'];
	$sending = $inputArr['sending'];
	...
	// query to get all the techies
	$get_techies = sql_queries::getTechies();
	...
	// $techies = json_encode($techies, JSON_PRETTY_PRINT);
	// doLogging("List of Techies: " . json_encode($techies, JSON_PRETTY_PRINT));
	...
	if ( $send_type == 'normal' ) {
		$headers .= "CC:" . additional_emails($send_type) . "\r\n";
		$headers .= "Bcc: " . bccEmailHeader($send_type) . "\r\n";
	} elseif ( $send_type == 'testcc') {
		$headers .= "CC:" . additional_emails($send_type) . "\r\n";
	} else {
		// Nothing to set
	...
	foreach ($techies as $techie){
		...
		if ( $continueScript ) {
			...
			// var_dump('Primary-Tech: ' . $techie["primarytech"]);
			// doLogging('Primary-Tech: ' . $techie["primarytech"]);
				...
				if ( $contNow ) {
					...
					if(is_array($resultsX)){
						...
						foreach($resultsX as $results){
							...
							$last  = (new DateTime('last day of this month'))->format('Y-m-d');

							// var_dump("CLIENT: " . $results[1]);

							$totalCalculationSQL = "SELECT SUM(bill_hrs) as 'LoggedHours' 
								FROM msi_billing 
								WHERE msi_billing.`date` >= '".$first."' 
								AND msi_billing.`date` <= '".$last."' 
								AND msi_billing.client = ".$results[1]." 
								AND msi_billing.type = 'Contract' 
								GROUP BY msi_billing.client;";
							...
							if ( empty($totalCalculation)) {
								$total = 0;
							} else {
								$total = $totalCalculation[0][0];
							}

							$arr = [];
							$arr['client'] = $results[0]; // Client Name
							$arr['contract_hours'] = $results[2]; // Contract Hours
							$arr['used_hours'] = $total; // Used Hours

							// $results[2] => Contract Hours
							// $total => Used Hours

							$message .= weekly_email_v2::hoursColorCoding($arr);

							// <td class=\'client\'>0.00</td>
							// <td class=\'client\'>0.00</td>
						...
						if ( $ptechieMail === null || strlen($ptechieMail) == 0 ){
							...
							if ( checkNameAndEmail($pritechie, $to) ) { 
								...
								var_dump('....ToEmail....' . $to . '<br />\n');
								var_dump('send_type: ' . $send_type);
...
class weekly_email_v2 {
	private static $testing;
	private static $localTesting;
	private static $totalUsed = 0.00;
	private static $contractHours = 0.00;
	...
	public static function hoursColorCoding( $arr ) {

		$clientStr = '';
		$contractHoursStr = '';
		$usedHoursStr = '';

		$progressMarker = self::getMonthlyProgress();

		$progressMarkerInHours = $progressMarker / 100 * $arr['contract_hours'];


		$clientStr = '<td class=\'client\' style="color: blue;">'.$arr['client'].' </td>';
		$contractHoursStr = '<td class=\'client\' style="color: black;">'.$arr['contract_hours'].' </td>';

		if ( $progressMarkerInHours > $arr['used_hours'] ) {
			$usedHoursStr = '<td class=\'client\' style="color: '.self::$colors['blue'].';">'.$arr['used_hours'] . ' </td>';
		} else {
			$usedHoursStr = '<td class=\'client\' style="color: '.self::$colors['red'].' ;">'.$arr['used_hours'] . ' </td>';
		}

		return '<tr style="background-color:lavender;padding:3px;">' . $clientStr . $contractHoursStr .$usedHoursStr.'</tr>';

	}

	private static function getMonthlyProgress() {

		$month = (int) date('n');
		$day = (int) date('j');

		// echo "Today is the $day" . date('S') . " day of the month" . "<br /><br />";
		$daysInMonth = cal_days_in_month(CAL_GREGORIAN, $month, date('Y'));

		// echo "There are $daysInMonth days in this month" . "<br /><br />";
		$progress = ($day / $daysInMonth) * 100;
	...
	// setters and getters
	public static function setTotalUsed($number) { self::$totalUsed = $number; }
	public static function setContractHours($number) { self::$contractHours = $number; }
	public static function setClient($name) { self::$client = $name; }

	public static function getTotalUsed() { return self::$totalUsed; }
	public static function getContractHours()	 { return self::$contractHours; }
	public static function getClient() { return self::$client; }

	private static function additional_emails ($testing) {
	...
	private static function bccEmailHeader ($testing) {
		if ( $testing ) {
			return "";
		} else {
